...
fn main() {
    ...
    benchmark(
        ...
        slice,
        algo_name,
        |data: &[u8], seed: i64| -> u64 { gxhash64(data, seed) },
    );
    ...
    benchmark(
        ...
        slice,
        "xxhash",
        |data: &[u8], seed: u64| -> u64 { twox_hash::xxh3::hash64_with_seed(data, seed) },
    );
    ...
    benchmark(
        ...
        slice,
        "ahash",
        |data: &[u8], _: i32| -> u64 { ahash_hasher.hash_one(data) },
    );
    ...
    benchmark(
        ...
        slice,
        "t1ha0",
        |data: &[u8], seed: u64| -> u64 { t1ha::t1ha0(data, seed) },
    );
    ...
    benchmark(
        ...
        slice,
        "seahash",
        |data: &[u8], seed: u64| -> u64 { seahash::hash_seeded(data, seed, 0, 0, 0) },
    );
    ...
    S: Default + TryFrom<u128> + TryInto<usize>,
{
    ...
    for i in 2.. {
        ...
        let throughput = (len as f64)
            / (1024f64
                * 1024f64
                * (total_duration.as_secs_f64() / runs as f64 / ITERATIONS as f64));

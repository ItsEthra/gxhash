...
fn benchmark_for_string(c: &mut Criterion, string: &str) {
    ...
    group.bench_function("Default Hasher", |b| b.iter(|| set.insert(string)));

    let mut set = GxHashSet::default();
    group.bench_function("GxHash", |b| b.iter(|| set.insert(string)));

    let mut set = HashSet::<&str, BuildHasherDefault<xxh3::Hash64>>::default();
    group.bench_function("XxHash", |b| b.iter(|| set.insert(string)));

    let mut set = FnvHashSet::default();
    group.bench_function("FNV-1a", |b| b.iter(|| set.insert(string)));
